...
CreateThreadPool(PTHREAD_POOL *threadPool, STATUS(*ProcessElement)(_In_ LPVOID elementToProcess))
{
	...
	_threadPool = (PTHREAD_POOL)HeapAlloc(GetProcessHeap(),0,sizeof(THREAD_POOL));
	if (NULL == _threadPool)
	{
	...
	if (SUCCESS != status)
	{
		HeapFree(GetProcessHeap(),0,_threadPool);
		_threadPool = NULL;
		goto Exit;
	}
...
DestroyThreadPool(PTHREAD_POOL *threadPool)
{
	...
	HeapFree(GetProcessHeap(),0,_threadPool->threadsHandle);
	_threadPool->threadsHandle = NULL;
	HeapFree(GetProcessHeap(),0,_threadPool);
	_threadPool->threadsHandle = NULL;
Exit:
...
Start(PTHREAD_POOL threadPool, INT nWorkers)
{
	...
	threadPool->threadsHandle = (HANDLE*)HeapAlloc(GetProcessHeap(),0,nWorkers * sizeof(HANDLE));
	...
	{
		params = (PARAMS_THREAD_POOL*)HeapAlloc(GetProcessHeap(),0,sizeof(PARAMS_THREAD_POOL));
...
Worker(LPVOID params)
{
	...
	HeapFree(GetProcessHeap(),0,(PARAMS_THREAD_POOL*)params);
